...
class Rai4Generator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {    
		...
		val outputPath = segments.subList(1, segments.size - 1).join("/")
	    
		for (element : resource.allContents.toIterable.filter(EObject)) {
			switch element {
				CassandraService: {
					generateCassandraConfigFiles(element, fsa, outputPath)
					generateCassandraRackConfigFiles(element, fsa, outputPath)
					generateCassandraScriptFiles(element, fsa, outputPath)
				}
				KafkaService: {
					generateKafkaConfigFile(element, fsa, outputPath)
					generateKafkaScriptFile(element, fsa, outputPath)
				}
				KafkaWorkloadStreamData: {
					generateTopicScript(element, fsa, outputPath)
				}
				StormService: {
					generateStormConfigFile(element, fsa, outputPath)
					generateStormScriptFile(element, fsa, outputPath)
				}
				Workflow: {
					generateWorkflowFiles(element, fsa, outputPath)
				}
				ZookeeperService: {
					generateZookeeperConfigFile(element, fsa, outputPath)
					generateZookeeperScriptFile(element, fsa, outputPath)
					generateZookeeperIdFile(element, fsa, outputPath)
				}
			}
		}
	}
	...
	// Cassandra Service
	def void generateCassandraConfigFiles(CassandraService cassandraService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''cassandra/cassandra«cassandraService.id».yaml''', configContent)
	}

	def void generateCassandraRackConfigFiles(CassandraService cassandraService, IFileSystemAccess2 fsa, String outputPath) {
		...
		for (entry : rackConfigs.entrySet) {
			fsa.generateFile(outputPath + '''cassandra/cassandra-rackcd.properties''', entry.value.toString)
		}
	}

	def void generateCassandraScriptFiles(CassandraService cassandraService, IFileSystemAccess2 fsa, String outputPath) {
		...
		for (node : (cassandraService.host as ProcessingNodeCluster).nodes) {
			...
			fsa.generateFile(outputPath + '''cassandra/cassandra«cassandraService.id».sh''', scriptContent)
		}
	}
	...
	// Kafka Service
	def void generateKafkaConfigFile(KafkaService kafkaService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''kafka/kafka«kafkaService.commId».properties''', configContent)
	}
	...
	def void generateKafkaScriptFile(KafkaService kafkaService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''kafka/kafka«kafkaService.commId».sh''', scriptContent)
	}

	// Kafka Workload Stream Data
	def void generateTopicScript(KafkaWorkloadStreamData kafkaStream, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''kafkaworkloadstreamdata/«scriptName»''', scriptContent)
		...
		if (kafkaStream.deploymentConfig instanceof OrchestratorDeploymentConf) {
			...
			'''
			fsa.generateFile(outputPath + '''kafkaworkloadstreamdata/helm_«scriptName»''', helmContent)
		}
	}

	private def String generateNodeScriptContent(KafkaWorkloadStreamData stream, String bootstrapServer,
		NodeDeploymentConf nodeConf) {
		'''
			#!/bin/bash
			cd «nodeConf.scriptFolderPath»
	...
	// Storm Service
	def void generateStormConfigFile(StormService stormService, IFileSystemAccess2 fsa, String outputPath) {
		val configContent = '''
			# Storm Configuration
		...
		'''
		fsa.generateFile(outputPath + "storm/storm.yaml", configContent)
	}
	...
	def void generateStormScriptFile(StormService stormService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + "storm/scripts/stormNimbus" + stormService.id + ".sh", nimbusScript)
		fsa.generateFile(outputPath + "storm/scripts/stormSupervisor" + stormService.id + ".sh", supervisorScript)
		fsa.generateFile(outputPath + "storm/scripts/stormUI" + stormService.id + ".sh", uiScript)
	}
	...
	// Workflow
	def void generateWorkflowFiles(Workflow workflow, IFileSystemAccess2 fsa, String outputPath) {
		for (streamData : workflow.ownedStreamData) {
			if (streamData instanceof FlowStreamData) {
				generateFlowStreamData(streamData as FlowStreamData, fsa, outputPath)
			}
		}

		for (taskExecutor : workflow.ownedTaskExecutors) {
			generateTaskExecutor(taskExecutor, fsa, outputPath)
		}

		// Generar configuración según el tipo de despliegue
		if (workflow.scheduler.host !== null) {
			generateWorkflowHostDeployment(workflow, fsa, outputPath)
		} else {
			generateWorkflowOrchestratorDeployment(workflow, fsa, outputPath)
		}
	}

	def private void generateWorkflowHostDeployment(Workflow workflow, IFileSystemAccess2 fsa, String outputPath) {
		...
		switch (workflow.deploymentConfig) {
			NodeDeploymentConf case (server instanceof StormService): {
				...
				fsa.generateFile(outputPath + fileName, scriptContent)

				// Generar archivo de artefacto
				generateArtifactFile(workflow, nodeConf, stormService, fsa, outputPath)
			}
		}
	}

	def private void generateWorkflowOrchestratorDeployment(Workflow workflow, IFileSystemAccess2 fsa, String outputPath) {
	...
	def private void generateArtifactFile(Workflow workflow, NodeDeploymentConf nodeConf, StormService stormService,
		IFileSystemAccess2 fsa, String outputPath) {
		val artifactContent = '''
		...
		fsa.generateFile(outputPath + '''workflow/artifacts/«nodeConf.artifactName».properties''', artifactContent)
	}

	def private void generateFlowStreamData(FlowStreamData flowData, IFileSystemAccess2 fsa, String outputPath) {
		// TODO: Implementar la generación de FlowStreamData
	}

	def private void generateTaskExecutor(TaskExecutor taskExecutor, IFileSystemAccess2 fsa, String outputPath) {
		// TODO: Implementar la generación de TaskExecutor
	}

	// Zookeeper Service
	def void generateZookeeperConfigFile(ZookeeperService zkService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''zookeeper/zoo«zkService.serverId».cfg''', configContent)
	}

	def void generateZookeeperIdFile(ZookeeperService zkService, IFileSystemAccess2 fsa, String outputPath) {
		val idContent = zkService.serverId.toString

		fsa.generateFile(outputPath + '''zookeeper/myid''', idContent)
	}

	def void generateZookeeperScriptFile(ZookeeperService zkService, IFileSystemAccess2 fsa, String outputPath) {
		...
		fsa.generateFile(outputPath + '''zookeeper/zkServer«zkService.serverId».sh''', scriptContent)
	}
