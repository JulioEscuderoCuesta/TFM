...
public class Rai4Generator extends AbstractGenerator {
  ...
  public void doGenerate(final Resource resource, final IFileSystemAccess2 fsa, final IGeneratorContext context) {
    final List<String> segments = resource.getURI().segmentsList();
    int _size = segments.size();
    int _minus = (_size - 1);
    final String outputPath = IterableExtensions.join(segments.subList(1, _minus), "/");
    ...
    for (final EObject element : _filter) {
      ...
      if (element instanceof CassandraService) {
        _matched=true;
        this.generateCassandraConfigFiles(((CassandraService)element), fsa, outputPath);
        this.generateCassandraRackConfigFiles(((CassandraService)element), fsa, outputPath);
        this.generateCassandraScriptFiles(((CassandraService)element), fsa, outputPath);
      }
      if (!_matched) {
        if (element instanceof KafkaService) {
          _matched=true;
          this.generateKafkaConfigFile(((KafkaService)element), fsa, outputPath);
          this.generateKafkaScriptFile(((KafkaService)element), fsa, outputPath);
        }
      }
      if (!_matched) {
        if (element instanceof KafkaWorkloadStreamData) {
          _matched=true;
          this.generateTopicScript(((KafkaWorkloadStreamData)element), fsa, outputPath);
        }
      }
      if (!_matched) {
        if (element instanceof StormService) {
          _matched=true;
          this.generateStormConfigFile(((StormService)element), fsa, outputPath);
          this.generateStormScriptFile(((StormService)element), fsa, outputPath);
        }
      }
      if (!_matched) {
        if (element instanceof Workflow) {
          _matched=true;
          this.generateWorkflowFiles(((Workflow)element), fsa, outputPath);
        }
      }
      if (!_matched) {
        if (element instanceof ZookeeperService) {
          _matched=true;
          this.generateZookeeperConfigFile(((ZookeeperService)element), fsa, outputPath);
          this.generateZookeeperScriptFile(((ZookeeperService)element), fsa, outputPath);
          this.generateZookeeperIdFile(((ZookeeperService)element), fsa, outputPath);
        }
      }
    }
  }

  public void generateCassandraConfigFiles(final CassandraService cassandraService, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    String _plus = (outputPath + _builder_1);
    fsa.generateFile(_plus, configContent);
  }

  public void generateCassandraRackConfigFiles(final CassandraService cassandraService, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    for (final Map.Entry<String, String> entry : _entrySet) {
      ...
      String _plus = (outputPath + _builder);
      fsa.generateFile(_plus, entry.getValue().toString());
    }
  }

  public void generateCassandraScriptFiles(final CassandraService cassandraService, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    for (final ProcessingNode node : _nodes) {
      {
        ...
        String _plus = (outputPath + _builder_1);
        fsa.generateFile(_plus, scriptContent);
      }
    }
  }
  ...
  public void generateKafkaConfigFile(final KafkaService kafkaService, final IFileSystemAccess2 fsa, final String outputPath) {
  ...
  public void generateKafkaScriptFile(final KafkaService kafkaService, final IFileSystemAccess2 fsa, final String outputPath) {
    DeploymentConfiguration _deploymentConfig = kafkaService.getDeploymentConfig();
    boolean _not = (!(_deploymentConfig instanceof NodeDeploymentConf));
    if (_not) {
      return;
  ...
  public void generateTopicScript(final KafkaWorkloadStreamData kafkaStream, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    String _plus = (outputPath + _builder_2);
    fsa.generateFile(_plus, scriptContent);
    DeploymentConfiguration _deploymentConfig_1 = kafkaStream.getDeploymentConfig();
    if ((_deploymentConfig_1 instanceof OrchestratorDeploymentConf)) {
      ...
      String _plus_1 = (outputPath + _builder_4);
      fsa.generateFile(_plus_1, helmContent);
    }
  }
  ...
  public void generateStormConfigFile(final StormService stormService, final IFileSystemAccess2 fsa, final String outputPath) {
    StringConcatenation _builder = new StringConcatenation();
  ...
  public void generateStormScriptFile(final StormService stormService, final IFileSystemAccess2 fsa, final String outputPath) {
  ...
  public void generateWorkflowFiles(final Workflow workflow, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    for (final WorkflowStreamData streamData : _ownedStreamData) {
      if ((streamData instanceof FlowStreamData)) {
        this.generateFlowStreamData(((FlowStreamData) streamData), fsa, outputPath);
      }
    }
    ...
    for (final TaskExecutor taskExecutor : _ownedTaskExecutors) {
      this.generateTaskExecutor(taskExecutor, fsa, outputPath);
    }
    ProcessingResourceCluster _host = workflow.getScheduler().getHost();
    boolean _tripleNotEquals = (_host != null);
    if (_tripleNotEquals) {
      this.generateWorkflowHostDeployment(workflow, fsa, outputPath);
    } else {
      this.generateWorkflowOrchestratorDeployment(workflow, fsa, outputPath);
    }
  }

  private void generateWorkflowHostDeployment(final Workflow workflow, final IFileSystemAccess2 fsa, final String outputPath) {
    ...
    if (_deploymentConfig instanceof NodeDeploymentConf) {
      if ((server instanceof StormService)) {
        ...
        this.generateArtifactFile(workflow, nodeConf, stormService, fsa, outputPath);
      }
    }
  }

  private void generateWorkflowOrchestratorDeployment(final Workflow workflow, final IFileSystemAccess2 fsa, final String outputPath) {
  }
  ...
  private void generateArtifactFile(final Workflow workflow, final NodeDeploymentConf nodeConf, final StormService stormService, final IFileSystemAccess2 fsa, final String outputPath) {
    StringConcatenation _builder = new StringConcatenation();
    ...
    _builder_1.append(".properties");
    String _plus = (outputPath + _builder_1);
    fsa.generateFile(_plus, artifactContent);
  }

  private void generateFlowStreamData(final FlowStreamData flowData, final IFileSystemAccess2 fsa, final String outputPath) {
  }

  private void generateTaskExecutor(final TaskExecutor taskExecutor, final IFileSystemAccess2 fsa, final String outputPath) {
  }

  public void generateZookeeperConfigFile(final ZookeeperService zkService, final IFileSystemAccess2 fsa, final String outputPath) {
  ...
  public void generateZookeeperIdFile(final ZookeeperService zkService, final IFileSystemAccess2 fsa, final String outputPath) {
    final String idContent = Integer.valueOf(zkService.getServerId()).toString();
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("zookeeper/myid");
    String _plus = (outputPath + _builder);
    fsa.generateFile(_plus, idContent);
  }

  public void generateZookeeperScriptFile(final ZookeeperService zkService, final IFileSystemAccess2 fsa, final String outputPath) {
